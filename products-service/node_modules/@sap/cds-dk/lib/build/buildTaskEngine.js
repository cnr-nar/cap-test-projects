...
class BuildTaskEngine {
    ...
    async processTasks(tasks) {
        ...
        this.logger.log(`building project [${cds.root}], clean [${this.buildOptions.clean}]`)
        this.logger.log(`cds-dk [${require('../../package.json').version}], cds [${cds.version}], compiler [${cds.compiler.version()}], home [${cds.home}]\n`)

        if (!tasks) {
            const buildTaskFactory = new BuildTaskFactory(this.buildOptions)
            tasks = await buildTaskFactory.getTasks()
        }
        ...
        // use resolved tasks
        this.providerFactory.context.tasks = handlers.map(handler => handler.task)
    ...
    _createHandler(task) {
        const handler = this.providerFactory.createHandler(task)
        handler.init()
     ...
     */
    _getLogLevel() {
        return this.buildOptions["log-level"] || cds.env["log-level"]
    }
}
