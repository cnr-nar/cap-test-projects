...
const { hasJavaNature, getDefaultModelOptions, BuildError, hasOptionValue, isClassicMtx } = require('../util')
...
Upgrade to @sap/cds-mtxs following the migration guide https://cap.cloud.sap/docs/guides/multitenancy/old-mtx-migration or install @sap/cds-dk version 6 instead.`)
    ...
    async lookupTasks(tasks) {
        return this._createTasks(tasks, tasks.length > 0)
    }
    ...
    async applyTaskDefaults(task) {
        ...
        BuildTaskProviderInternal._setDefaultModel(task)
    }

    async _createTasks(tasks, addRequiredTasks) {
        ...
        if (Array.isArray(db) && db.length > 0) {
            db = BuildTaskProviderInternal._getModuleFolder(db) || null
            if (!db && !addRequiredTasks) { // log once
                this.logger.debug("No database module found")
            }
        }
        if (Array.isArray(srv) && srv.length > 0) {
            srv = BuildTaskProviderInternal._getModuleFolder(srv) || null
    ...
    _createMtxTask(src) {
        ...
        if (BuildTaskProviderInternal._isMtxs()) {
            ...
            if (!BuildTaskProviderInternal._hasJavaNature(src)) {
    ...
    static _setDefaultModel(task) {
        ...
        if (taskModelPaths?.length) {
            if (!hasOptionValue(task.options?.[DEFAULT_BUILT_IN_MODELS], false)) {
                ...
                defaultModelPaths = getDefaultModelOptions().filter(p => p.match('@sap/cds'))
            }
        } else {
